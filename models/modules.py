...
class Decoder(nn.Module):
    def __init__(
        self, in_dim, out_dim, num_rels, layer_num, low_memory=False, decoder="rgat"
    ):
        ...
        if self.decoder == "rgcn":
            ...
            self.decoder_f = GNN(
                self.h_dim, self.h_dim, layer_num=self.d_layer_num, gnn=self.decoder
            )
        elif self.decoder == "gat":
            self.decoder_f = GAT(
                self.h_dim, self.h_dim, num_head=2, layer_num=self.d_layer_num
            )
...
class Encoder(nn.Module):
    def __init__(
        self, in_dim, out_dim, num_rels, layer_num, low_memory=False, encoder_f="rgat"
    ):
        ...
        if self.encoder == "rgcn":
            ...
            self.aggregator = GNN(
                self.h_dim, self.h_dim, layer_num=self.a_layer_num, gnn=self.encoder
            )
...
class GNN(nn.Module):
    def __init__(
        ...
        if self.gnn == "rgcn":
            ...
            self.layer = nn.ModuleList(
                RGATLayer(self.h_dim, self.h_dim) for _ in range(self.layer_num)
            )
...
class RGATLayer(nn.Module):
    ...
    def reduce_func(self, nodes):
        ...
        # equation (4)
        h = torch.sum(alpha * nodes.mailbox["z"], dim=1) + torch.mm(
            nodes.data["z"], self.loop_weight
        )
        return {"h": h}
