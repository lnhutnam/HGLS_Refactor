...
from models.layers import UnionRGCNLayer, RGCNBlockLayer
from models.basics import BaseRGCN
...
class RecurrentRGCN(nn.Module):
    def __init__(
        ...
        self.w1 = torch.nn.Parameter(
            torch.Tensor(self.h_dim, self.h_dim), requires_grad=True
        ).float()
    ...
    def forward(self, g_list, static_graph=None, device=None):
        ...
        for i, g in enumerate(g_list):
            ...
            if i == 0:
                x_input = torch.cat(
                    (self.emb_rel[0 : self.num_rels * 2], x_input), dim=1
                )
                self.h_0 = self.relation_cell_1(
                    x_input, self.emb_rel[0 : self.num_rels * 2]
                )  # 第1层输入
    ...
    def predict(self, test_graph, num_rels, static_graph, test_triplets, use_cuda):
        with torch.no_grad():
            ...
            embedding = (
                F.normalize(evolve_embs[-1]) if self.layer_norm else evolve_embs[-1]
            )
    ...
    def get_loss(self, glist, triples, static_graph, device):
        ...
        if self.entity_prediction:
            scores_ob = self.decoder_ob.forward(pre_emb, r_emb, all_triples).view(
                -1, self.num_ents
            )
            ...
            if self.discount == 1:
                for time_step, evolve_emb in enumerate(evolve_embs):
                    ...
                    if self.layer_norm:
                        ...
                        c = torch.norm(static_emb, p=2, dim=1) * torch.norm(
                            evolve_emb, p=2, dim=1
                        )
