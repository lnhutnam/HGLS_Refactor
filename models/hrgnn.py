...
class HRGNN(nn.Module):
    ...
    def forward(
        self, data_list, node_id_new=None, time_gap=None, device=None, mode="test"
    ):
        ...
        if self.encoder == "ori":
            ...
            if self.encoder == "regcn":
                ...
                sub_e_graph.apply_edges(
                    lambda edges: {"norm": edges.dst["norm"] * edges.src["norm"]}
                )
                h[pre_id] = self.aggregator(
                    sub_e_graph,
                    h[pre_id],
                    [
                        self.rel_embedding.weight[0 : self.num_rels * 2],
                        self.rel_embedding.weight[0 : self.num_rels * 2],
                    ],
                    1,
                )
        ...
        if self.decoder == "regcn":
            ...
            sub_d_graph.apply_edges(
                lambda edges: {"norm": edges.dst["norm"] * edges.src["norm"]}
            )
            new_feature[pre_id] = self.decoder_f(
                sub_d_graph, h[pre_id], [self.rel_embedding.weight] * self.d_layer_num
            )
        elif self.decoder in ["rgat_r1"]:
            sub_d_graph = data_list["sub_d_graph"].to(device)
            pre_id = data_list["pre_d_nid"]
            sub_d_graph.edata["r_h"] = self.rel_embedding(
                sub_d_graph.edata["etype"]
            ) + self.pos_decoder(sub_d_graph.edata["e_r"])
...
class GNN(nn.Module):
    def __init__(
        ...
        if self.gnn == "rgcn":
            ...
            self.layer = nn.ModuleList(
                GraphConv(self.h_dim, self.h_dim, norm="both", activation=F.relu)
                for _ in range(self.layer_num)
            )
        elif self.gnn == "rgat_r1":
            self.layer = nn.ModuleList(
                RGATLayer(
                    self.h_dim, self.h_dim, self.feat_drop, self.attn_drop, self.gnn
                )
                for _ in range(self.layer_num)
            )
...
class RGATLayer(nn.Module):
    ...
    def edge_attention(self, edges):
        ...
        if self.gnn in ["rgat", "rgat_r", "rgat1", "rgat_r1"]:
            ...
            e = (
                (self.w1(edges.src["z"]) + edges.data["r_h"]) * self.w2(edges.dst["z"])
            ).sum(1) / torch.sqrt(torch.tensor(self.h_dim).float())
    ...
    def reduce_func(self, nodes):
        ...
        # equation (4)
        h = self.feat_drop(
            torch.sum(alpha * (nodes.mailbox["z"] + nodes.mailbox["r_h"]), dim=1)
            + torch.mm(nodes.data["z"], self.loop_weight)
        )
        return {"h": h}
