...
from models.rrgcn import RGCNCell, RecurrentRGCN
from models.hrgnn import GNN, HRGNN
from models.decoders import ConvTransE, ConvTransR

from utils.graph_utils import build_sub_graph
...
class HGLS(nn.Module):
    def __init__(
        ...
        if self.gnn == "regcn":
            ...
            self.rgcn = GNN(
                self.h_dim,
                self.h_dim,
                layer_num=long_con["a_layer_num"],
                gnn=self.gnn,
                attn_drop=0.0,
                feat_drop=0.2,
            )
        if self.short:
            self.model_r = RecurrentRGCN(
                num_ents=num_nodes, num_rels=num_rels, gnn=self.gnn, **short_con
            )
        ...
        if self.long:
            self.model_t = HRGNN(
                graph=graph, num_nodes=num_nodes, num_rels=num_rels, **long_con
            )
        ...
        self.decoder_ob = ConvTransE(
            num_nodes,
            h_dim,
            short_con["input_dropout"],
            short_con["hidden_dropout"],
    ...
    def forward(
        ...
        if self.short:
            ...
            history_glist = [
                build_sub_graph(self.num_nodes, self.num_rels, snap, device)
                for snap in input_list
            ]
            evolve_embs, static_emb, r_emb, _, _ = self.model_r(
                history_glist, device=device
            )
            pre_emb = F.normalize(evolve_embs[-1])
        if self.long:
            new_embedding = F.normalize(
                self.model_t(data_list, node_id_new, time_gap, device, mode)
            )
        ...
        if self.long and self.short:
            ...
            if self.fuse == "con":
                ...
                pre_emb, e_cof = self.fuse_attention(
                    pre_emb, new_embedding, self.en_embedding.weight
                )
            elif self.fuse == "att1":
        ...
        scores_ob = self.decoder_ob.forward(pre_emb, r_emb, all_triples, mode).view(
            -1, self.num_nodes
        )
        loss_ent += self.loss_e(scores_ob, all_triples[:, 2])

        if self.relation_prediction:
            score_rel = self.rdecoder.forward(pre_emb, r_emb, all_triples, mode).view(
                -1, self.num_rels * 2
            )
    ...
    def fuse_attention(self, s_embedding, l_embedding, o_embedding):
        ...
        en_embedding = (
            aff[:, 0].unsqueeze(1) * s_embedding + aff[:, 1].unsqueeze(1) * l_embedding
        )
